...
$(document).ready(function() {
	...
	$("#js-create-frm").on('submit', function(event) {
		...
		if (!validName($("#js-name-create").val())) {
		...
		if (!validPassword($("#js-password-create").val())) {
...
function responseProcess(data) {
	...
	case "AUTH_REQUIREMENT":
		window.location.replace(context + "/login?token=false");
		break;
	case "INPUT_INVALID":
		alert("Input invalid");
		break;
...
function formToJson(formData) {
	...
	for (var i = 0; i < array.length; i++) {
		...
		temp[1] = temp[1].replace(new RegExp('%26', 'g'),'&');
		if (i == array.length - 1) {
