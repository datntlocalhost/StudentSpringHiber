...
<#macro formInput path attributes="" fieldType="text">
    <@bind path/>
    <input type="${fieldType}"  name="${status.expression}" value="<#if fieldType!="password"><#autoesc>${stringStatusValue}</#autoesc></#if>" ${attributes?no_esc}<@closeTag/>
</#macro>
...
<#macro formSchoolSelect path options attributes="">
    <@bind path/>
    <select name="${status.expression}" ${attributes?no_esc}>
        <option value=""></option>
        <#if options?is_hash>
            <#list options?keys as value>
        ...
        <#else> 
            <#list options as value>
            <option value="${value.schoolId}">${value.schoolName}</option>
            </#list>
        </#if>
    </select>
</#macro>

<#macro formSchoolCode path options type="" attributes="">
    <@bind path/>
    <select name="${status.expression}" ${attributes?no_esc}>
        <option value=""></option>
        <#if type == 'code' >
            <#list options as value>
            <option value="${value.schoolCode}">${value.schoolCode}</option>
            </#list>
        <#else> 
            <#list options as value>
            <option value="${value.schoolId}">${value.schoolName}</option>
...
<#macro formSexRadio path attributes="">
    <@bind path/>
    <input type="radio" name="${status.expression}" value="" checked ${attributes?no_esc}<@closeTag/>
    <label style="margin-right: 7px;">All</label>
    
    <input type="radio" name="${status.expression}" value="Male" ${attributes?no_esc}<@closeTag/>
    <label style="margin-right: 7px;">Male</label>
    
    <input type="radio" name="${status.expression}" value="Female" ${attributes?no_esc}<@closeTag/>
    <label style="margin-right: 7px;">Female</label>
    
</#macro>

<#macro formSexSelect path attributes="">
	<@bind path/>
	<select name="${status.expression}" ${attributes?no_esc}>
...
<#macro formYearSelect path attributes="">
	<@bind path/>
	<select name="${status.expression}" ${attributes?no_esc}>
